% the script performs the declipping experiment from the survey
% 
...
for i = 1:length(audio_files)
    for j = 1:length(input_SDRs)
        ...
        % load
        fprintf(['Loading audio ''', audio_files{i} , '.wav''\n']);
        ...
        for m = 1:length(lambdaC)
            ...
            % waveforms
            S.clean = data;
            savename = [directory, '/', audio_files{i}(1:3), '_', num2str(input_SDRs(j), '%02d'), '.mat'];
            if isfile(savename)
